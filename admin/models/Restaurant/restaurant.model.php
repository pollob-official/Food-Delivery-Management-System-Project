...
class Restaurant extends Model implements JsonSerializable{
	...
	public static function find_order_by_restuarant_id($id){
		global $db,$tx;
		$result =$db->query("select id,customer_id,rider_id,restaurant_id,delivery_address,total_amount,delivery_fee,tax_amount,coupon_id,tracking_id,payment_status,created_at,updated_at,version from {$tx}Orders where restaurant_id='$id'");
		$data=[];
		while($restaurant=$result->fetch_object()){
			$data[]=$restaurant;
		}
